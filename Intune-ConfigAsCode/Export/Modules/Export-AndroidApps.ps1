function Export-AndroidApps {
    ...
    Write-Host "Exporting Android Apps..." -ForegroundColor Cyan

    $ExportPath = Join-Path $RepositoryRoot "Applications\Android"
    ...
    $AndroidApps = $Response.value | Where-Object {
        $_.'@odata.type' -in @(
    ...
    Write-Host ""
    Write-Host "Exported $Count Android App(s)." -ForegroundColor Green
    Write-Host ""
}